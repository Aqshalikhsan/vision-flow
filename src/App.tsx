  ...
  const downloadColabNotebook = async () => {
    ...
    try {
      ...
      const source = [
        ...
        "def download_setup(url, filename):",
        '    request = urllib.request.Request(url, headers={"User-Agent": "Google-Colab"})',
        '    with urllib.request.urlopen(request) as response, open(filename, "wb") as output:',
        "        output.write(response.read())",
        'download_setup(SERVER + "/api/training-workers/setup/visionflow_worker.py", "visionflow_worker.py")',
        'download_setup(SERVER + "/api/training-workers/setup/requirements.txt", "requirements.txt")',
