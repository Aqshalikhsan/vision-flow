...
const authTourUrl = process.env.VISIONFLOW_AUTH_TOUR_URL;
...
test("Gmail OTP first login tours the real E2E project through detected deployment image", async ({
  ...
  test.skip(
    !authTourUrl,
    "Set VISIONFLOW_AUTH_TOUR_URL to run the deployment-specific auth tour",
  );
  await page.goto(authTourUrl!);

  await expect(
    page.getByRole("heading", {
      name: /Buat akun workspace|Selamat datang kembali/,
    }),
  ).toBeVisible();
  ...
  await expect(
    page.getByRole("heading", { name: "Kenali project E2E COCO8" }),
  ).toBeVisible({
    timeout: 20_000,
  });
  await expect(page).toHaveURL(
    /e2e-coco8-detection-20260828-153649-2637\/project/,
  );
  ...
  await expect(page).toHaveURL(
    /e2e-coco8-detection-20260828-153649-2637\/deploy/,
  );
  await expect(page.locator(".inference-box").first()).toBeVisible({
    timeout: 120_000,
  });
  await expect(page.locator(".inference-box").first()).toContainText(/\d+%/);
  await page.screenshot({
    path: ".tmp/auth-tour-deployment.png",
    fullPage: true,
  });

  await page.getByRole("button", { name: /Selesai/ }).click();
  await expect(
    page.getByRole("heading", { name: "Jalankan inference objek nyata" }),
  ).toBeHidden();
  await page.reload();
  await expect(
    page.getByRole("heading", { name: "Jalankan inference objek nyata" }),
